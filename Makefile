...
test: compile
	...
	@printf '  %-42s' "test_awk.ady (-t then -r)"; \
	    tmp=$$(mktemp -d); \
	    XDG_CACHE_HOME=$$tmp $(PY2NIM) -t $(EXDIR)/test_awk.ady >/dev/null 2>&1 \
	      && XDG_CACHE_HOME=$$tmp $(PY2NIM) $(EXDIR)/test_awk.ady -r </dev/null >/dev/null 2>&1 \
	      && { echo OK; rm -rf $$tmp; } \
...
clean:
	...
	@# The same place py2nim writes to: XDG_CACHE_HOME when it is set,
	@# ~/.cache otherwise.
